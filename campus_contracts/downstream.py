...
class RoundAssignment(Strict):
    ...
    #: Сколько обходов человек прошёл за всё время. Кабинет это число только
    #: показывает: считает его АСПиРС по своей истории обходов, а посчитать его
    #: у себя кабинет не может — его собственные таблицы обхода живут
    #: `RETENTION_DAYS` дней (семь) и дальше вычищаются, то есть «за всё время»
    #: у него физически нет.
    #:
    #: Пусто — «не знаю», а не ноль: пока отправитель не выложен, поля в
    #: сообщении нет вовсе, и показывать кабинету нечего.
    #:
    #: Между двумя сообщениями число может и уменьшиться: перераздача
    #: возвращает сданный подъезд в открытое состояние, и пройденным он быть
    #: перестаёт. Это зеркало чужого счёта, а не счётчик достижений —
    #: принимающая сторона заменяет прежнее значение, а не берёт максимум.
    rounds_walked: int | None = Field(default=None, ge=0)
